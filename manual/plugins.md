---
title: "Guía de Extensión y Creación de Plugins: nostromo"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de nostromo"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-nostromo-plugins)=
# Guía de Extensión y Plugins: nostromo

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`nostromo`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-nostromo-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`nostromo` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para nostromo`.
- **Punto de Entrada Oficial**: `nostromo.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-nostromo-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para nostromo."""
    name = "custom_rule"
...
Para que `nostromo` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."nostromo.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `nostromo` la reconozca:
...
nostromo plugins list
````

---

(manual-nostromo-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `nostromo` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = nostromo.ejecutar_analisis(
...
(manual-nostromo-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `nostromo` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar nostromo
        run: pip install -e ./nostromo
        
      - name: Ejecutar Auditoría
        run: nostromo check src/ include/ --json > reporte.json
````

---

(manual-nostromo-plugins-ejercicios)=
